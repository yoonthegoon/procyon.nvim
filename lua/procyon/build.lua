...
-- Get the directory where the procyon plugin is located
local function get_plugin_dir()
	return debug.getinfo(1).source:match("@?(.*/)"):gsub("/lua/procyon/$", "")
end
...
-- Turn procyon naming conventions into more common ANSI names
local function get_palette(variant)
	package.loaded["procyon/palettes/" .. variant] = nil
	local colors = require("procyon/palettes/" .. variant)
  -- stylua: ignore
  return {
    bg             = colors.a.bg,
    fg             = colors.a.fg,
...
local function build(terminals)
	for _, variant in ipairs({ "dark", "light" }) do
		...
		for term, attrs in pairs(terminals) do
			...
			fwrite(fmt, string.format("%s/procyon_%s%s", dir, variant, attrs.ext))
		end

		fwrite(vim.json.encode(palette), get_plugin_dir() .. string.format("/procyon_%s.json", variant))
	end
end
